...
import com.ibcs.desco.cs.service.VPAppHierHistoryService;
import com.ibcs.desco.cs.service.VehiclePermissionService;

@Controller
@PropertySource("classpath:common.properties")
public class VehiclePermissionController extends Constrants {
	
	// Service Class Object Creation
	@Autowired
	private VehiclePermissionService vehiclePermissionService;
	...
	@Autowired
	private VPAppHierHistoryService vpAppHierHistoryService;
	...
	@RequestMapping(value = "/cs/vehiclePermission.do", method = RequestMethod.GET)
	@PreAuthorize("isAuthenticated()")
	@PostAuthorize("hasPermission(#vehiclePermission, 'WRITE')")
	public ModelAndView vehiclePermission(VehiclePermission 
			vehiclePermission){
		// Declare date for year and month part of auto generated slip no 
		//Date entryTime = new Date();
		// Pass date value into auto generated slip no function
		/*String slipNo = vehiclePermissionSlipNo(entryTime);
		model.put("slipNo", slipNo);*/
		...
		return new ModelAndView("centralStore/csVehiclePermissionForm",model);
	}
	
	// Function for save vehicle permission form's data into database 
	@RequestMapping(value = "/cs/saveVehiclePermission.do", method = RequestMethod.POST)
	@PreAuthorize("isAuthenticated()")
	public String saveVehiclePermission(
			@ModelAttribute("vehiclePermissionUI") VehiclePermission 
			vehiclePermissionUI) throws ParseException{
		...
		VehiclePermission vehiclePermission = new VehiclePermission();
		
		// Get data from JSP page and set them into class 
		
		String descoDeptCode = department.getDescoCode();
		String slipNo = commonService
				.getOperationIdByPrefixAndSequenceName(
						vehiclePermissionPrefix, descoDeptCode,
						separator, "CS_VEHICLE_PERM_SEQ");
		vehiclePermission.setSlipNo(slipNo);
		
		vehiclePermission.setContractorName(vehiclePermissionUI.getContractorName());
		vehiclePermission.setEntryTime(entryTime);
		vehiclePermission.setEntryPurpose(vehiclePermissionUI.getEntryPurpose());
		vehiclePermission.setVehicleType(vehiclePermissionUI.getVehicleType());
		vehiclePermission.setVehicleNumber(vehiclePermissionUI.getVehicleNumber());
		vehiclePermission.setDrivingLicenceNo(vehiclePermissionUI.getDrivingLicenceNo());
		vehiclePermission.setRequisitionBy(vehiclePermissionUI.getRequisitionBy());
		...
		// Get all approval hierarchy on CS_VEHICLE_PERMISSION
		List<ApprovalHierarchy> approvalHierarchyList = approvalHierarchyService.
				getApprovalHierarchyByOperationName(CS_VEHICLE_PERMISSION);
		...
		// Sorting the founded state code on CS_VEHICLE_PERMISSION
		...
		ApprovalHierarchy approvalHierarchyInfo = approvalHierarchyService
						.getApprovalHierarchyByOperationNameAndSateCode(
								CS_VEHICLE_PERMISSION, stateCodes[0].toString());
		...
		if(flag == 1){
			VehiclePermissionApprovalHierarchyHistory vpApprovalHierarchyHistory = new VehiclePermissionApprovalHierarchyHistory();
			vpApprovalHierarchyHistory.setActRoleName(userRoleName);
			vpApprovalHierarchyHistory.setOperationId(vehiclePermission.getSlipNo());
			vpApprovalHierarchyHistory.setOperationName(CS_VEHICLE_PERMISSION);
			vpApprovalHierarchyHistory.setCreatedBy(vehiclePermission.getCreatedBy());
			vpApprovalHierarchyHistory.setCreatedDate(entryTime);
			if(stateCodes.length > 0){
				vpApprovalHierarchyHistory.setStateCode(stateCodes[0]);
				vpApprovalHierarchyHistory.setStateName(approvalHierarchyInfo.getStateName());
			}
			vpApprovalHierarchyHistory.setStatus(OPEN);
			vpApprovalHierarchyHistory.setActive(vehiclePermission.getActive());
			vpApprovalHierarchyHistory.setApprovalHeader(approvalHierarchyInfo.getApprovalHeader());
			vpApprovalHierarchyHistory.setDeptId(authUser.getDeptId());
			
			commonService.saveOrUpdateModelObjectToDB(vpApprovalHierarchyHistory);
		}
		commonService.saveOrUpdateModelObjectToDB(vehiclePermission);
		
		return "redirect:/cs/showVehiclePermission.do?id="+vehiclePermission.getId();
	}

	@RequestMapping(value = "/cs/vehiclePermissionList.do", method = RequestMethod.GET)
	@PreAuthorize("isAuthenticated()")
	public ModelAndView vehiclePermissionList(){
		...
		@SuppressWarnings("unchecked")
		List<VehiclePermissionApprovalHierarchyHistory> vpAppHierHistoryList = (List<VehiclePermissionApprovalHierarchyHistory>) (Object) commonService.
		getHierarcyHistoryListByDeptIdAndRoleNameAndStatus(
				"com.ibcs.desco.cs.model.VehiclePermissionApprovalHierarchyHistory", deptId, roleName, OPEN);
		
		String[] operationIdList = new String[vpAppHierHistoryList.size()];
		for (int i = 0; i < vpAppHierHistoryList.size(); i++) {
			operationIdList[i] = vpAppHierHistoryList.get(i)
					.getOperationId();
		}
		
		List<VehiclePermission> vehiclePermissionList = vehiclePermissionService.
				getVehiclePermissionListByOperationId(operationIdList);
		...
		if (rolePrefix.equals("ROLE_CS")) {
			return new ModelAndView("centralStore/csVehiclePermissionList", model);
		} else {
			return new ModelAndView("centralStore/csVehiclePermissionList");
		}
	}
	
	@RequestMapping(value = "/cs/showVehiclePermission.do", method = RequestMethod.GET)
	@PreAuthorize("isAuthenticated()")
	public ModelAndView vehiclePermissionShow(VehiclePermission vehiclePermission){
		// Get the ROLE name
		String roleName = commonService.getAuthRoleName();
		...
		VehiclePermission vehiclePermissionShow = vehiclePermissionService.
				getVehiclePermission(id);
		...
		@SuppressWarnings("unchecked")
		List<VehiclePermissionApprovalHierarchyHistory> vpAppHierHistoryDoneList = (List<VehiclePermissionApprovalHierarchyHistory>) (Object) commonService.
				getHierarcyHistoryListByOperationNameOperationIdAndStatus(
						"VehiclePermissionApprovalHierarchyHistory", CS_VEHICLE_PERMISSION, operationId, DONE);
		
		// Get current status of the vehicle slip no
		if (!vpAppHierHistoryDoneList.isEmpty()) {
			currentStatus = vpAppHierHistoryDoneList.get(vpAppHierHistoryDoneList.size() - 1).getStateName();
		} else {
		...
		//@SuppressWarnings("unchecked")
		@SuppressWarnings("unchecked")
		List<VehiclePermissionApprovalHierarchyHistory> vpAppHierHistoryOpenList = (List<VehiclePermissionApprovalHierarchyHistory>) (Object) commonService.
				getHierarcyHistoryListByOperationNameOperationIdAndStatus(
						"VehiclePermissionApprovalHierarchyHistory", CS_VEHICLE_PERMISSION, operationId, OPEN);
		
		int currentStateCode = vpAppHierHistoryOpenList.get(vpAppHierHistoryOpenList.size() - 1).getStateCode();
		...
		List<Roles> roleObjectList = (List<Roles>) (Object) commonService.
				getObjectListByAnyColumnValueList(
						"com.ibcs.desco.admin.model.Roles", "role_id", roleIdList);
		...
		List<ApprovalHierarchy> approvalHierarchyList = approvalHierarchyService.
				getApprovalHierarchyByOperationNameAndRoleName(CS_VEHICLE_PERMISSION, roleNameList);
		
		// Send to upper Authority of same department
		List<ApprovalHierarchy> nextManReqProcs = new ArrayList<ApprovalHierarchy>();
		for(int countStateCodes = 0; countStateCodes < approvalHierarchyList.size(); countStateCodes++){
		...
		for(int countBackStateCodes = 0; countBackStateCodes < approvalHierarchyList.size(); countBackStateCodes++){
			if(approvalHierarchyList.get(countBackStateCodes).getStateCode() < currentStateCode){
				backManRcvProcs.add(approvalHierarchyList.get(countBackStateCodes));
			}
		}
		...
		// Button Name Define
		if(!vpAppHierHistoryOpenList.isEmpty() && vpAppHierHistoryOpenList != null){
			
			// Get current state code
			int stateCode = vpAppHierHistoryOpenList.get(vpAppHierHistoryOpenList.size() - 1).getStateCode();
			
			// Decide for return or not
			returnState = vpAppHierHistoryOpenList.get(vpAppHierHistoryOpenList.size() - 1).getReturn_state();
			
			// Get next Approval Hierarchy
			ApprovalHierarchy approvalHierarchy = null;{
				approvalHierarchy = approvalHierarchyService.
						getApprovalHierarchyByOperationNameAndSateCode(
								CS_VEHICLE_PERMISSION, stateCode + "");
		...
		model.put("dateFormate", simpleDateFormat.format(vehiclePermissionShow.getEntryTime()));
		...
		if (rolePrefix.equals("ROLE_CS")) {
			return new ModelAndView("centralStore/csVehiclePermissionShow", model);
		} else {
			return new ModelAndView("centralStore/csVehiclePermissionShow", model);
		}		
	}
	
	@RequestMapping(value = "/cs/vehiclePermissionSubmitApproved", method = RequestMethod.GET)
	public String vehiclePermissionSubmitApproved(Model model, 
		...
		if(vehiclePermissionUI.getReturnState() != null){
			@SuppressWarnings("unchecked")
			List<VehiclePermissionApprovalHierarchyHistory> vpAppHierHistoryList = 
					(List<VehiclePermissionApprovalHierarchyHistory>) (Object) 
					commonService.getObjectListByAnyColumn(
							"VehiclePermissionApprovalHierarchyHistory", 
			...
			VehiclePermissionApprovalHierarchyHistory vpApprovalHierarchyHistory = vpAppHierHistoryService.
					getvpApprovalHierarchyHistory(ids[0]);
			...
			ApprovalHierarchy approvalHierarchy = approvalHierarchyService.
					getApprovalHierarchyByOperationNameAndSateCode(
							CS_VEHICLE_PERMISSION, currentStateCode + "");
			vpApprovalHierarchyHistory.setStatus(DONE);
			vpApprovalHierarchyHistory.setCreatedBy(userName);
			vpApprovalHierarchyHistory.setRemarks(vehiclePermissionUI.getRemarks());
			vpApprovalHierarchyHistory.setApprovalHeader(approvalHierarchy.getApprovalHeader());
			
			commonService.saveOrUpdateModelObjectToDB(vpApprovalHierarchyHistory);
			
			// Get next state code and all info from approval hierarchy history
			VehiclePermissionApprovalHierarchyHistory vpAppHierHistoryNextState = new VehiclePermissionApprovalHierarchyHistory();
			ApprovalHierarchy approvalHierarchyNextState = approvalHierarchyService.
					getApprovalHierarchyByOperationNameAndSateCode(
							CS_VEHICLE_PERMISSION, vehiclePermissionUI.getReturnState());
			...
			vpAppHierHistoryNextState.setStateName(
					approvalHierarchyNextState.getStateName());
			vpAppHierHistoryNextState.setStateCode(
					Integer.parseInt(
							vehiclePermissionUI.getReturnState()));
			vpAppHierHistoryNextState.setId(null);
			vpAppHierHistoryNextState.setOperationId(
					vehiclePermissionUI.getSlipNo());
			vpAppHierHistoryNextState.setOperationName(
					approvalHierarchyNextState.getOperationName());
			vpAppHierHistoryNextState.setActRoleName(
					approvalHierarchyNextState.getRoleName());
			vpAppHierHistoryNextState.setApprovalHeader(
					approvalHierarchyNextState.getApprovalHeader());
			vpAppHierHistoryNextState.setDeptId(deptId);
			vpAppHierHistoryNextState.setcDeptName(department.getDeptName());
			vpAppHierHistoryNextState.setcDesignation(authUser.getDesignation());
			
			commonService.saveOrUpdateModelObjectToDB(vpAppHierHistoryNextState);
			
		}else{
			//Get vehicle permission list against slip No
			String operationId = vehiclePermissionUI.getSlipNo();
			
			// Get all state code from approval hierarchy and sort descending order to get highest state code
			List<ApprovalHierarchy> approvalHierarchyList  = approvalHierarchyService.
					getApprovalHierarchyByOperationName(CS_VEHICLE_PERMISSION);
			...
			@SuppressWarnings("unchecked")
			List<VehiclePermissionApprovalHierarchyHistory> approvalHierarchyHistoryList = 
					(List<VehiclePermissionApprovalHierarchyHistory>) (Object) 
					commonService.getObjectListByAnyColumn(
							"VehiclePermissionApprovalHierarchyHistory", "operationId", operationId);
			...
			// Get current state code and all info from approval hierarchy history
			VehiclePermissionApprovalHierarchyHistory approvalHierarchyHistory = vpAppHierHistoryService.
					getvpApprovalHierarchyHistory(ids[0]);
						
			int currentStateCode = approvalHierarchyHistory.getStateCode();
			int nextStateCode = 0;
			
			// Searching next state code and decision for send this vehicle permission to next person
			for (int state : stateCodes) {
				// If next state code grater than current state code than this process will go to next person
				if(state > currentStateCode){
					nextStateCode = state;
					ApprovalHierarchy approvalHierarchy = approvalHierarchyService.
							getApprovalHierarchyByOperationNameAndSateCode(
									CS_VEHICLE_PERMISSION, nextStateCode + "");
					...
					if (r1.equals(r2)){
						...
						@SuppressWarnings("unchecked")
						List<AuthUser> userList = (List<AuthUser>) (Object) commonService.
								getObjectListByAnyColumn("com.ibcs.desco.admin.model.AuthUser", 
										"roleid", role.getRole_id() + "");
						Departments departments = (Departments) commonService.
								getAnObjectByAnyUniqueColumn("Departments", "deptId", userList.get(0).getDeptId());
						approvalHierarchyHistory.setDeptId(departments.getDeptId());
						approvalHierarchyHistory.setcDeptName(departments.getDeptName());						
					}
				...
				// If next state code equal to current state code than this process will done for login user
				if (state == currentStateCode) {
					ApprovalHierarchy approvalHierarchy = approvalHierarchyService.
							getApprovalHierarchyByOperationNameAndSateCode(CS_VEHICLE_PERMISSION, state + "");
					approvalHierarchyHistory.setStatus(DONE);
					approvalHierarchyHistory.setCreatedBy(userName);
					approvalHierarchyHistory.setRemarks(vehiclePermissionUI.getRemarks());
					approvalHierarchyHistory.setApprovalHeader(approvalHierarchy.getApprovalHeader());
					
					commonService.saveOrUpdateModelObjectToDB(approvalHierarchyHistory);
				}
				
				// If next state code is last as approval hierarchy than this process
				...
				if(currentStateCode == stateCodes[(stateCodes.length) - 1]) {
					...
					List<ApprovalHierarchy> approvalHierarchiesListDB = approvalHierarchyService.
							getApprovalHierarchyByOperationName(CS_VEHICLE_PERMISSION);
					Integer[] vpStateCodes = new Integer[approvalHierarchiesListDB.size()];
					
					for (int i = 0; i < approvalHierarchiesListDB.size(); i++) {
						vpStateCodes[i] = approvalHierarchiesListDB.get(i).getStateCode();
					}
					...
					ApprovalHierarchy vpApprovalHierarchy = approvalHierarchyService.
							getApprovalHierarchyByOperationNameAndSateCode(
									CS_VEHICLE_PERMISSION, vpStateCodes[0].toString());
					
					VehiclePermissionApprovalHierarchyHistory vpAppHierarchyHistory = new VehiclePermissionApprovalHierarchyHistory();
					vpAppHierarchyHistory.setActRoleName(vpApprovalHierarchy.getRoleName());
					vpAppHierarchyHistory.setOperationId(operationId);
					vpAppHierarchyHistory.setOperationName(CS_VEHICLE_PERMISSION);
					...
					if (stateCodes.length > 0) {
						...
						vpAppHierarchyHistory.setStateName(vpApprovalHierarchy.getStateName());
					}
					vpAppHierarchyHistory.setStatus(OPEN);
					vpAppHierarchyHistory.setActive(true);
					
					commonService.saveOrUpdateModelObjectToDB(vpAppHierarchyHistory);
					
					model.addAttribute("operationId", operationId);
					System.out.println("Print a slip");
					//return "centralStore/csVehiclePermissionReport";
					String param1 = vehiclePermissionUI.getSlipNo();
					return "redirect:/cs/csVehiclePermissionReport.do?param1="+param1;
				}				
			}
		}
		return "redirect:/cs/vehiclePermissionList.do";
		
	}
	
	@RequestMapping(value="/cs/csVehiclePermissionReport.do", method=RequestMethod.GET)
	public ModelAndView csVehiclePermissionReport( String param1 ) {
		Map<String, Object> model = new HashMap<String, Object>();
		model.put("param1", param1);
		return new ModelAndView("centralStore/csVehiclePermissionReport", model);
	}
	
	@RequestMapping(value="/cs/vehiclePermissionReport.do", method=RequestMethod.GET)
	public ModelAndView vehiclePermissionReport() {
		return new ModelAndView("report/csVehiclePermissionReport");
	}
	
	
	@RequestMapping(value = "/cs/vehiclePermissionSendTo.do", method = RequestMethod.GET)
	@PreAuthorize("isAuthenticated()")
	public String vehiclePermissionSendTo(Model model,
			@ModelAttribute("vehiclePermissionUI") VehiclePermission vehiclePermission) {
		...
		@SuppressWarnings("unchecked")
		List<VehiclePermissionApprovalHierarchyHistory> approvalHierarchyHistoryList = (List<VehiclePermissionApprovalHierarchyHistory>) (Object) commonService
				.getObjectListByAnyColumn(
						"com.ibcs.desco.cs.model.VehiclePermissionApprovalHierarchyHistory",
						"operationId", slipNo);
		...
		Arrays.sort(ids, Collections.reverseOrder());
		// get current State Code and all info from approval hierarchy history
		VehiclePermissionApprovalHierarchyHistory approvalHierarchyHistory = vpAppHierHistoryService.getvpApprovalHierarchyHistory(ids[0]);
		int currentStateCode = approvalHierarchyHistory.getStateCode();
		...
		// get Next State Code and all info from approval hierarchy history
		VehiclePermissionApprovalHierarchyHistory approvalHierarchyHistoryNextState = new VehiclePermissionApprovalHierarchyHistory();
	...
	@RequestMapping(value = "/cs/vehiclePermissionBackTo.do", method = RequestMethod.GET)
	@PreAuthorize("isAuthenticated()")
	public String vehiclePermissionBackTo(Model model,
			@ModelAttribute("vehiclePermissionUI") VehiclePermission vehiclePermission) {
		String slipNo = vehiclePermission.getSlipNo();
		String remarks = vehiclePermission.getRemarks();
		...
		// get Next State Code and all info from approval hierarchy history
		VehiclePermissionApprovalHierarchyHistory approvalHierarchyHistoryBackState = new VehiclePermissionApprovalHierarchyHistory();
		ApprovalHierarchy approvalHierarchyBackSate = approvalHierarchyService
				.getApprovalHierarchyByOperationNameAndSateCode(
						CS_VEHICLE_PERMISSION, backStateCode + "");
