...
public class WsToCsTransformerReturnController extends Constrants {
	...
	@RequestMapping(value = "/returnSlip/wsTransformerReturnSlipForm.do", method = RequestMethod.GET)
	...
	public ModelAndView getReturnSlip(PndContractor pdContractor) {
		...
		try {
			...
			List<ItemMaster> itemList = itemInventoryService.getItemListByCategoryId(300);
			...
			return new ModelAndView("workshop/transformer/wsToCsReturnSlipForm", model);
		} catch (Exception e) {
			model.put("errorMsg", e);
			return new ModelAndView("workshop/transformer/errorProject", model);
		}
	}
	...
	public String returnSlipMstSave(Model model,
		...
		returnSlipMst.setSenderStore(ContentType.WS_CN_XFORMER.toString());
		...
		return "redirect:/wsx/returnSlipShow.do?id=" + returnSlipMstdb.getId();
	...
	public boolean addWsCsReturnDtls(ReturnSlipMstDtl returnSlipMstDtl, ReturnSlipMst returnSlipMst, String roleName,
		...
		List<ApprovalHierarchy> approvalHierarchyList = approvalHierarchyService
				.getApprovalHierarchyByOperationName(WS_CS_X_RETURN);
		...
		if (stateCodes.length > 0) {
			approvalHierarchy = approvalHierarchyService.getApprovalHierarchyByOperationNameAndSateCode(WS_CS_X_RETURN,
	...
	public void addReturnSlipHierarchyHistory(ReturnSlipMst returnSlipMst, ApprovalHierarchy approvalHierarchy,
		...
		WsCsXReturnSlipApprovalHierarchyHistory approvalHierarchyHistory = new WsCsXReturnSlipApprovalHierarchyHistory();
		...
		approvalHierarchyHistory.setOperationName(WS_CS_X_RETURN);
	...
	public ModelAndView showReturnnListofLocal() {
		...
		List<WsCsXReturnSlipApprovalHierarchyHistory> storeSlipApprovalHierarchyHistoryList = (List<WsCsXReturnSlipApprovalHierarchyHistory>) (Object) commonService
		...
		if (rolePrefix.equals("ROLE_WO")) {
			return new ModelAndView("workshop/transformer/wsToCsReturnSlipList", model);
		} else if (rolePrefix.equals("ROLE_WS")) {
			return new ModelAndView("workshop/transformer/wsReturnSlipList", model);
		} else if (rolePrefix.equals("ROLE_CS")) {
			return new ModelAndView("workshop/transformer/csReturnSlipList", model);
		} else {
			model.put("errorMsg",
	...
	public ModelAndView returnSlipShow(ReturnSlipMst returnSlipMst) {
		...
		List<WsCsXReturnSlipApprovalHierarchyHistory> rsApprovalHierarchyHistoryList = (List<WsCsXReturnSlipApprovalHierarchyHistory>) (Object) commonService
				.getHierarcyHistoryListByOperationNameOperationIdAndStatus("WsCsXReturnSlipApprovalHierarchyHistory",
						WS_CS_X_RETURN, operationId, DONE);
		...
		List<WsCsXReturnSlipApprovalHierarchyHistory> rsApprovalHierarchyHistoryOpenList = (List<WsCsXReturnSlipApprovalHierarchyHistory>) (Object) commonService
				.getHierarcyHistoryListByOperationNameOperationIdAndStatus("WsCsXReturnSlipApprovalHierarchyHistory",
						WS_CS_X_RETURN, operationId, OPEN);
		...
		List<ApprovalHierarchy> approveHeirchyList = approvalHierarchyService
				.getApprovalHierarchyByOperationNameAndRoleName(WS_CS_X_RETURN, roleNameList);
		...
		if (!rsApprovalHierarchyHistoryOpenList.isEmpty() && rsApprovalHierarchyHistoryOpenList != null) {
			...
			ApprovalHierarchy approveHeirarchy = approvalHierarchyService
					.getApprovalHierarchyByOperationNameAndSateCode(WS_CS_X_RETURN, stateCode + "");
		...
		if(returnSlipMstdb.getOthers()!=null){
		String s = returnSlipMstdb.getOthers();
		
		String[] sSplit=s.split(",");
		
		for(TransformerRegister trans:transformerRegList){
			
			for(int i=0; i<sSplit.length; i++){
			if (trans.getTransformerSerialNo()
					.contains(sSplit[i])) {
				trans.setReturnSlipNo("1");
				
			}
			
			}
			transformerRegisterList.add(trans);
		}
		}else{transformerRegisterList.addAll(transformerRegList);}
		//------------------------------------------------
		...
		String[] sSplit=s.split(",");
		for(int i=0; i<sSplit.length; i++){
			TransformerRegister tr=wSTransformerService.getRegisteredTransformer(sSplit[i].toString());
			newStr+=sSplit[i]+"("+tr.getJobNo()+")";
			if(i<sSplit.length-1){
				newStr+=",";}
		}
		}
		returnSlipMstdb.setOthers(newStr);
		...
		String rolePrefix = roleName.substring(0, 7);
		if (rolePrefix.equals("ROLE_WO")) {
			return new ModelAndView("workshop/transformer/wsToCsReturnSlipShow", model);
		} else if (rolePrefix.equals("ROLE_WS")) {
			
			model.put("transformerList", transformerRegisterList);
			
			return new ModelAndView("workshop/transformer/wsReturnSlipShow", model);
		} else if (rolePrefix.equals("ROLE_CS")) {
			...
			return new ModelAndView("workshop/transformer/csReturnSlipShow", model);
	...
	public String returnSlipForItemSubmitApproved(Model model, ReturnSlipMstDtl returnSlipMstDtl) {
		...
		if (!returnSlipMstDtl.getReturn_state().equals("") || returnSlipMstDtl.getReturn_state().length() > 0) {
			List<WsCsXReturnSlipApprovalHierarchyHistory> approvalHierarchyHistoryList = (List<WsCsXReturnSlipApprovalHierarchyHistory>) (Object) commonService
					.getObjectListByAnyColumn("WsCsXReturnSlipApprovalHierarchyHistory", "operationId",
			...
			WsCsXReturnSlipApprovalHierarchyHistory approvalHierarchyHistory = (WsCsXReturnSlipApprovalHierarchyHistory) commonService
					.getAnObjectByAnyUniqueColumn("WsCsXReturnSlipApprovalHierarchyHistory", "id", ids[0] + "");
			...
			ApprovalHierarchy approvalHierarchy = approvalHierarchyService
					.getApprovalHierarchyByOperationNameAndSateCode(WS_CS_X_RETURN, currentStateCode + "");
			...
			WsCsXReturnSlipApprovalHierarchyHistory approvalHierarchyHistoryNextState = new WsCsXReturnSlipApprovalHierarchyHistory();
			ApprovalHierarchy approvalHierarchyNextSate = approvalHierarchyService
					.getApprovalHierarchyByOperationNameAndSateCode(WS_CS_X_RETURN, returnSlipMstDtl.getReturn_state());
			...
			List<WsCsXReturnSlipApprovalHierarchyHistory> approvalHierarchyHistoryList = (List<WsCsXReturnSlipApprovalHierarchyHistory>) (Object) commonService
					.getObjectListByAnyColumn("WsCsXReturnSlipApprovalHierarchyHistory", "operationId", operationId);
			...
			for (int state : stateCodes) {
				...
				if (state > currentStateCode) {
					nextStateCode = state;
					ApprovalHierarchy approvalHierarchy = approvalHierarchyService
							.getApprovalHierarchyByOperationNameAndSateCode(WS_CS_X_RETURN, nextStateCode + "");
				...
				if (state == currentStateCode) {
					ApprovalHierarchy approvalHierarchy = approvalHierarchyService
							.getApprovalHierarchyByOperationNameAndSateCode(WS_CS_X_RETURN, state + "");
					approvalHierarchyHistory.setStatus(DONE);
					approvalHierarchyHistory.setCreatedBy(userName);
					approvalHierarchyHistory.setcEmpId(authUser.getEmpId());
					approvalHierarchyHistory.setcEmpFullName(authUser.getName());
				...
				if (currentStateCode == stateCodes[(stateCodes.length) - 1]) {
					...
					if (csStoreTicketMstdb == null) {
						...
						csStoreTicketMst.setStoreTicketType(WS_CS_X_RETURN);
						...
						/*csStoreTicketMst.setReceivedBy(returnSlipMst
								.getReceivedBy());*/
						...
						return "workshop/transformer/wsToCsXReturnSlipReport";
					}

				}
			}
		}
		return "redirect:/wsx/returnSlip/List.do";
	}
	...
	public String centralSotreReceivedItemSendTo(
		...
		List<WsCsXReturnSlipApprovalHierarchyHistory> approvalHierarchyHistoryList = (List<WsCsXReturnSlipApprovalHierarchyHistory>) (Object) commonService
				.getObjectListByAnyColumn(
						"WsCsXReturnSlipApprovalHierarchyHistory",
						"operationId", rrNo);
		...
		ApprovalHierarchy approvalHierarchy = approvalHierarchyService
				.getApprovalHierarchyByOperationNameAndSateCode(
						WS_CS_X_RETURN, currentStateCode + "");
		approvalHierarchyHistory.setStatus(DONE);
		approvalHierarchyHistory.setModifiedBy(userName);
		approvalHierarchyHistory.setModifiedDate(new Date());
		approvalHierarchyHistory.setCreatedBy(userName);
	...
	public String centralSotreReceivedItemBackTo(
		...
		// get Next State Code and all info from approval hierarchy history
		WsCsXReturnSlipApprovalHierarchyHistory approvalHierarchyHistoryBackState = new WsCsXReturnSlipApprovalHierarchyHistory();
		ApprovalHierarchy approvalHierarchyBackSate = approvalHierarchyService
				.getApprovalHierarchyByOperationNameAndSateCode(
						WS_CS_X_RETURN, backStateCode + "");
