function forwardToUpper(stateCode) {
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		$(".qtyNewServiceable").each(function() {
			qtyNewServiceable.push($(this).val());
		});
		
		$(".qtyRecServiceable").each(function() {
			qtyRecServiceable.push($(this).val());
		});
		
		$(".qtyUnServiceable").each(function() {
			qtyUnServiceable.push($(this).val());
		});
		
		$(".totalReturn").each(function() {
			totalReturn.push($(this).val());
		});	
		var params={
		...
		var path=$("#contextPath").val()+"/ss/returnSlip/sendTo.do";
...
function backToLower(stateCode) {
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		window.location = $("#contextPath").val()
				+ "/ss/returnSlip/backTo.do?returnSlipNo=" + returnSlipNo
...
function approveing() {
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		var justification = $('#justification').val();
		var returnSlipNo = $('#returnSlipNo').val();
		var return_state = $('#returnStateCode').val();	
		var dtlId= [];
		var qtyNewServiceable= [];
		var qtyRecServiceable=  [];
		var qtyUnServiceable=  [];
		...
		var path=$("#contextPath").val()+"/ss/returnSlip/itemReturnSlipApproved.do";
		postSubmit(path, params, "POST");
	}
								...
								$.ajax({
									...
									success : function(data) {
										...
										var r = $("#tblAppendGrid").attr(
												"data-rowid");
										$("#qtyNewServiceable" + r).val(
												sData.qtyNewServiceableRcv
														.toFixed(2));
										$("#qtyRecServiceable" + r).val(
												sData.qtyRecServiceableRcv
														.toFixed(2));
										$("#qtyUnServiceable" + r).val(
