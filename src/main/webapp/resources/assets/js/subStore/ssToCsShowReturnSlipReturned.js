function checkAndPostRequest(path, params, method) {

	var qtyRecServiceable = [];
	$(".qtyRecServiceable").each(function() {
		var th = $(this);
		qtyRecServiceable.push(th.text());
	});

	var qtyUnServiceable = [];
	$(".qtyUnServiceable").each(function() {
		var th = $(this);
		qtyUnServiceable.push(th.text());
	});

	var qtyUnServiceableRcv = [];
	$(".qtyUnServiceableRcv").each(function() {
		var th = $(this);
		qtyUnServiceableRcv.push(th.text());
	});

	var qtyNewServiceable = [];
	$(".qtyNewServiceable").each(function() {
		var th = $(this);
		qtyNewServiceable.push(th.text());
	});

	var qtyNewServiceableRcv = [];
	$(".qtyNewServiceableRcv").each(function() {
		var th = $(this);
		qtyNewServiceableRcv.push(th.text());
	});

	var qtyRecServiceableRcv = [];
	$(".qtyRecServiceableRcv").each(function() {
		var th = $(this);
		qtyRecServiceableRcv.push(th.text());
	});
	// Checking any Issued Qty is Greater?
	var flag = true;
	for (var i = 0; i < qtyNewServiceable.length; i++) {
		var nsQty = parseFloat($("#qtyNewServiceable" + i).text());
		var nsrQty = parseFloat($("#qtyNewServiceableRcv" + i).text());

		var rsQty = parseFloat($("#qtyRecServiceable" + i).text());
		var rsrQty = parseFloat($("#qtyRecServiceableRcv" + i).text());

		var usQty = parseFloat($("#qtyUnServiceable" + i).text());
		var usrQty = parseFloat($("#qtyUnServiceableRcv" + i).text());
	...
	if (flag) {
		...
		alert("Please Set Location wise Quantity Properly");
	}
