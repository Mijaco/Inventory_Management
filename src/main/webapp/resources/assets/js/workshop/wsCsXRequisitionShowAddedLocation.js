...
/*function forwardToUpper(stateCode) {
	...
	window.location = $('#contextPath').val()
			+ "/wsx/transformer/requisition/sendTo.do?requisitionNo=" + requisitionNo
...
function backToLower(stateCode) {
	...
	// "${pageContext.request.contextPath}/cs/itemRecieved/itemReceivedForwordTo.do?receivedReportNo="+param1+"$justification="+justification;
	window.location = $('#contextPath').val()
			+ "/wsx/transformer/requisition/backTo.do?requisitionNo=" + requisitionNo
			+ "&justification=" + justification + "&stateCode=" + stateCode;
}*/
function forwardToUpper(stateCode) {
	...
	var path=contextPath+"/wsx/transformer/requisition/sendTo.do";
...
function backToLower(stateCode) {
	...
	var path=contextPath+"/wsx/transformer/requisition/backTo.do";
...
function approveing() {
	...
	$(".quantityIssued").each(function() {
		reqIssueList.push($(this).val());
	});

	var cData = {
		requisitionNo : $("#requisitionNo").val(),
		headOfAccount : reqDtlIdList,
		quantityIssued : reqIssueList,
		justification : justification,
		return_state : returnStateCode
	}
	
	var path=contextPath+"/wsx/transformer/csItemRequisitionSubmitApproved.do";
...
/*function approveSubCsRequisition() {
	...
	if (flag) {
		window.location = $('#contextPath').val()
				+ "/wsx/transformer/itemRequisitionSubmitApproved.do?requisitionNo="
