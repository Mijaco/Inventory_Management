...
function forwardTo(stateCode) {
	...
	window.location = $('#contextPath').val()
			+ "/ws/requisition/sendTo.do?requisitionNo=" + requisitionNo
			+ "&justification=" + justification + "&stateCode=" + stateCode;
}

function backTo(stateCode) {
	var justification = $('#justification').val();
	var requisitionNo = $('#requisitionNo').val();
	// window.location =
	// "${pageContext.request.contextPath}/cs/itemRecieved/itemReceivedForwordTo.do?receivedReportNo="+param1+"$justification="+justification;
	window.location = $('#contextPath').val()
			+ "/ws/requisition/backTo.do?requisitionNo=" + requisitionNo
			+ "&justification=" + justification + "&stateCode=" + stateCode;
}
/*function forwardTo(stateCode) {
	...
	var path=contextPath+"/ws/requisition/sendTo.do";
...
function backTo(stateCode) {
	...
	var path=contextPath+"/ws/requisition/backTo.do";
...
function approveing() {
	...
	var path=contextPath+"/ws/csItemRequisitionSubmitApproved.do";
