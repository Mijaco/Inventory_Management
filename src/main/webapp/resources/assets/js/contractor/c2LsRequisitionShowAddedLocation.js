...
function editItemLocation(itemCode, rowid) {
	...
	$
			.ajax({
				type : "POST",
				contentType : "application/json; charset=utf-8",
				url : $('#contextPath').val() + "/ls/getTemplocation.do",
				data : cDataJsonString,
				success : function(data) {
					$('#dynamicTable').append(
...
function updateLocationQty(pk, index) {
	...
	$.ajax({
		type : "POST",
		contentType : "application/json; charset=utf-8",
		url : $('#contextPath').val() + "/ls/updateLocationQty.do",
		data : cDataJsonString,
		success : function(data) {
			alert(data);
		},
...
function deleteLocationQty(pk, index) {
	...
	$.ajax({
		type : "POST",
		contentType : "application/json; charset=utf-8",
		url : $('#contextPath').val() + "/ls/deleteLocationQty.do",
		data : cDataJsonString,
		success : function(data) {
			alert(data);
...
function forwardToUpper(stateCode) {
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		var quantityIssuedNS = [];
		$(".quantityIssuedNS").each(function() {
			var th = $(this);
			quantityIssuedNS.push(th.val());
		});

		var quantityIssuedRS = [];
		$(".quantityIssuedRS").each(function() {
			var th = $(this);
			quantityIssuedRS.push(th.val());
		});

		if (quantityIssuedNS.length > 0 || quantityIssuedRS.length > 0) {
		window.location = $('#contextPath').val()
				+ "/c2ls/requisition/sendTo.do?requisitionNo=" + requisitionNo
				+ "&justification=" + justification + "&stateCode=" + stateCode+ "&quantityIssuedNS="
				+ quantityIssuedNS + "&quantityIssuedRS=" + quantityIssuedRS;
		} else {
...
function backToLower(stateCode) {
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		if (quantityIssuedNS.length > 0 || quantityIssuedRS.length > 0) {
		window.location = $('#contextPath').val()
				+ "/c2ls/requisition/backTo.do?requisitionNo=" + requisitionNo
				+ "&justification=" + justification + "&stateCode=" + stateCode+ "&quantityIssuedNS="
				+ quantityIssuedNS + "&quantityIssuedRS=" + quantityIssuedRS;
		} else {
			alert("Issued Quantity can not be 0 or null");
...
function approveing() {
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		if (quantityIssuedNS.length > 0 || quantityIssuedRS.length > 0) {
			window.location = $('#contextPath').val()
					+ "/c2ls/itemRequisitionSubmitApproved.do?requisitionNo="
					+ requisitionNo + "&justification=" + justification
					+ "&return_state=" + returnStateCode + "&quantityIssuedNS="
					+ quantityIssuedNS + "&quantityIssuedRS=" + quantityIssuedRS;
		} else {
			alert("Issued Quantity can not be 0 or null");
		}
