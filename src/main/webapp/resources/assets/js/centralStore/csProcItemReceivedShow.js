...
function forwardToUpper(stateCode) {
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		window.location = $('#contextPath').val()
				+ "/cs/itemRecieved/itemReceivedSendTo.do?receivedReportNo="
				+ recieviedReportNo + "&justification=" + justification
				+ "&stateCode=" + stateCode;
	}
...
function backToLower(stateCode) {
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		var recieviedReportNo = $('#recieviedReportNo').val();
		window.location = $('#contextPath').val()
				+ "/cs/itemRecieved/itemReceivedBackTo.do?receivedReportNo="
				+ recieviedReportNo + "&justification=" + justification
				+ "&stateCode=" + stateCode;
	}
...
function approveing() {
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		window.location = $('#contextPath').val()
				+ "/cs/itemRecieved/itemReceivedSubmitApproved.do?receivedReportNo="
				+ recieviedReportNo + "&justification=" + justification
...
// Start of Code for Location Grid
								...
								$.ajax({
									...
									success : function(data) {
										...
										var r = $("#tblAppendGrid").attr(
												"data-rowid");
										var expectedQty = $('#expectedQty' + r).text();
										var reminingQuantity = $('#reminingQuantity' + r).val();
										
										$("#receivedQty" + r).text(
												sData.toFixed(3));
										var remQty = parseFloat(expectedQty) - sData;
										$('#reminingQuantity' + r).text(remQty);
