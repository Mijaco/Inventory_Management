function deleteItem(id) {
	if( confirm( "Do you want to delete this item?" ) == true ) {
		...
		var path = baseURL + "/mps/deletePackageDtl.do";
		var param = {
				'id' : id
		}
...
function checkAnnexureNoFromShow() {
	var prevAnnexureNo = $('#prevAnnexureNo').val();
	var annexureNo = $('#annexureNo').val();
	var contextPath = $('#contextPath').val();
	var saveButton = $('#saveButton');
	
	if( prevAnnexureNo == annexureNo ) {
...
//Master Info. Dialog Box
$(function() {
	$("#masterInfoDialogeBox").dialog({
			autoOpen : false,
			closeOnEscape : false,
			modal : true,
			draggable : false,
...
function openMasterDialog() {
	$('#masterInfoDialogeBox').removeClass('hide');
	$(".ui-dialog-titlebar").hide();
	$("#masterInfoDialogeBox").dialog("open");
}
...
$(function() {
	$("#myDialogeBox").dialog({
			...
			buttons : [ {
				...
				click : function() {
					// send post request
					$("#saveMultipleProcurementPackage").submit();
					// $('#myDialogeBox').empty();
				}
			}, {
				text : 'Close',
