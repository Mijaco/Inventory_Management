function forwardToUpper(stateCode) {
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		window.location = $('#contextPath').val() + "/ls/returnSlip/sendToLsToSs.do?returnSlipNo="
				+ returnSlipNo
				+ "&justification="
				+ justification
				+ "&stateCode="
...
function backToLower(stateCode) {
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		// window.location =
		// "${pageContext.request.contextPath}/cs/itemRecieved/itemReceivedForwordTo.do?receivedReportNo="+param1+"$justification="+justification;
		window.location = $('#contextPath').val() + "/ls/returnSlip/backToLsToSs.do?returnSlipNo="
				+ returnSlipNo
				+ "&justification="
				+ justification
				+ "&stateCode="
...
function approveing() {
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		window.location = $('#contextPath').val() + "/ls/itemReturnSlipApprovedLsToSs.do?returnSlipNo="
				+ returnSlipNo
				+ "&justification="
				+ justification
							...
							click : function(evt) {
								...
								$.ajax({
									...
									success : function(data) {
										...
										$("#qtyNewServiceable" + r).text(sData.qtyNewServiceableRcv.toFixed(3));
										$("#qtyRecServiceable" + r).text(sData.qtyRecServiceableRcv.toFixed(3));
										$("#qtyUnServiceable" + r).text(sData.qtyUnServiceableRcv.toFixed(3));
										$("#totalReturn" + r).text(sData.totalReturnRcv.toFixed(3));
										$("#myGrid").dialog("close");
										$('#tblAppendGrid').empty();
									},
									error : function(data) {
