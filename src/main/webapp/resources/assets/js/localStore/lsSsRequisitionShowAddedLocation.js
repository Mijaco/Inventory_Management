function forwardToUpper(stateCode) {
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		window.location = $('#contextPath').val()
				+ "/ls/requisition/sendToLocalToSub.do?requisitionNo="
				+ requisitionNo + "&justification=" + justification + "&stateCode="
				+ stateCode+ "&issueqty="
				+ issuedQtys;
		} else {
...
function backToLower(stateCode) {
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		if (flag) {
		window.location = $('#contextPath').val()
				+ "/ls/requisition/backToLocalToSub.do?requisitionNo="
				+ requisitionNo + "&justification=" + justification + "&stateCode="
				+ stateCode+ "&issueqty="
				+ issuedQtys;
		} else {
...
function approveLsSsRequisition() {
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		$('.justification').removeClass('hide');
		$('#justification').focus();
		return;
		...
		if (flag) {
			window.location = $('#contextPath').val()
					+ "/ls/itemRequisitionSubmitApprovedLocalToSub.do?requisitionNo="
					+ requisitionNo + "&justification=" + justification
					+ "&return_state=" + returnStateCode + "&issueqty="
					+ issuedQtys;
		} else {
			alert("Issued Quantity can not be greater than Required Quantity. Please reduce issued quantity from popup grid.");
		}
...
function ssrejectProcess() {
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		var path = contextPath + "/ls/ss/itemRequisitionReject.do";

		var flag = confirm("Do you want to reject this process?");
		if (flag == true) {
