				...
				click : function() {
					...
					$('.quantityRequired').each( function() {
						if( $(this).val() == null || $.trim( $(this).val() ) == '' || $.trim( $(this).val() ) == '0' || $.trim( $(this).val() ) == '0.0' || $.trim( $(this).val() ) == '0.00' || $.trim( $(this).val() ) == '0.000' ) {
							...
							$('#errQty'+sequence).removeClass('hide');
	...
	$(document)
			.on(
					...
					function(e) {
						...
						//errQty
						var errQtyDiv = childDiv
						.getElementsByTagName('div')[5], errQtyInput = receivedQtyDiv
						.getElementsByTagName('strong')[0];
						errQtyInput.setAttribute('id', 'errQty'
								+ newNum);
...
function editItem(id){	
	...
	$.ajax({								
		url : '${pageContext.request.contextPath}/ss/itemEditToSub.do',
		data : "{id:"+id+"}",
		contentType : "application/json",
		success : function(data) {									
