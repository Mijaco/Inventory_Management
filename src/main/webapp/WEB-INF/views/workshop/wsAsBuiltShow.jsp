...
<div class="container-fluid icon-box" style="background-color: #858585;">
	...
	<div class="row"
		...
		<!-- --------------------- -->
		<form id="myForm" action="${pageContext.request.contextPath}/ws/asBuilt/asBuiltSubmitApproved.do" method="POST">
		<div class="oe_title">
	...
	value="${asBuiltMst.createdDate}" pattern="dd-MM-yyyy hh:mm:ss a" /></td>
				</tr>
				<tr class="">
						...
						<c:forEach items="${asBuiltDtlList}"
							...
							<tr>
								...
								<td style="width:150px;"><input style="width:80px;" class="recServiceable"
											type="number" step="0.01" value="${asBuiltDtl.recServiceable}" name="recServiceable" onblur="getTotalCost(this)"
											id="recServiceable${loop.index}" /></td>
								<td><input class="form-control total"
											type="number" step="0.01" value="${asBuiltDtl.total}" name="total"
											id="total${loop.index}" /></td> --%>
											 <td><c:out value="${asBuiltDtl.materialsQuantity}" /><input type="hidden" value="${asBuiltDtl.materialsQuantity}" id="materialsQuantity${loop.index}" name="materialsQuantity" /></td>
										<td><c:out value="${asBuiltDtl.receivedQuantity}" /><input type="hidden" value="${asBuiltDtl.receivedQuantity}" id="receivedQuantity${loop.index}" name="receivedQuantity" /></td>
										<td><input	type="number" id="materialsConsume${loop.index}" value="${asBuiltDtl.materialsConsume}" name="materialsConsume" onblur="getTotalCost(this)" /></td>
										<td style="width:150px;"><input style="width:80px;"	type="number" step="0.001" value="${asBuiltDtl.materialsInHand}" name="materialsInHand" id="materialsInHand${loop.index}" readonly="readonly" />
...
<script
	src="${pageContext.request.contextPath}/resources/assets/js/workshop/wsAsBuiltShow.js"></script>
