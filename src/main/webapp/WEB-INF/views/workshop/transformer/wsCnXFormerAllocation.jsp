...
<div class="container-fluid icon-box" style="background-color: #858585;">
	...
	<div class="row"
		...
		<div class="col-sm-12 table-responsive">
			<c:if test="${empty wsCnAllocationList}">
			...
			<c:if test="${!empty wsCnAllocationList}">
				<table id="inventoryList" style=""
					...
					<tbody>
						<c:forEach items="${wsCnAllocationList}" var="cnAllocation"
							varStatus="loop">
							<tr>
								<td>${cnAllocation.workOrderNo}
									<input type="hidden" id="pk${loop.index}" value="${cnAllocation.id}" />
								</td>
								<td>${cnAllocation.contractorMst.contractorName}</td>
								<td><input type="number" min="0" readonly="readonly"
									id="repairQty1P${loop.index}" style="width: 70px"
									name="repairQty1P" value="${cnAllocation.repairQty1P}" /></td>
								<td><input type="number" min="0" readonly="readonly"
									id="repairQty3P${loop.index}" style="width: 70px"
									name="repairQty3P" value="${cnAllocation.repairQty3P}" /></td>
								<td><input type="number" min="0" readonly="readonly"
									id="preventiveQty1P${loop.index}" style="width: 70px"
									name="preventiveQty1P" value="${cnAllocation.preventiveQty1P}" /></td>
								<td><input type="number" min="0" readonly="readonly"
									id="preventiveQty3P${loop.index}" style="width: 70px"
									name="preventiveQty3P" value="${cnAllocation.preventiveQty3P}" /></td>
								<td><input type="text" id="remarks${loop.index}"
									readonly="readonly" name="remarks"
									placeholder="Write your remarks"
									value="${cnAllocation.remarks}" /></td>
								<td>
...
<script type="text/javascript">
	...
	function updateWsCnAllocation(n){
		...
		var path= contextPath + '/ws/xf/updateWsCnAllocation.do';
		 
		var cData = {
				id : id,
				repairQty1P :  $("#repairQty1P"+n).val(),
				repairQty3P : $("#repairQty3P"+n).val(),
				preventiveQty1P : $("#preventiveQty1P"+n).val(),
				preventiveQty3P : $("#preventiveQty3P"+n).val(),				
