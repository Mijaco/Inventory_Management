	...
	<div class="row" style="background-color: white; padding: 10px; padding-left: 20px; margin: 10px;">
		...
		<div class="col-sm-12 table-responsive">
			...
			<c:if test="${!empty cnWsRequisitionMstList}">
				...
				<table id="cnPdRequisitionListTable"
					...
					<tbody>
						<c:forEach items="${cnWsRequisitionMstList}"
							var="cnWsCsRequisitionMst">
							<tr>
								<td><a
									href="${pageContext.request.contextPath}/cnws/showXformerMatsRequisition.do?id=${cnWsCsRequisitionMst.id}"
									style="text-decoration: none;"><c:out
											value="${cnWsCsRequisitionMst.requisitionNo}" /></a></td>
								<td><c:out value="${cnWsCsRequisitionMst.identerDesignation}" /></td>
								<td><fmt:formatDate
										value="${cnWsCsRequisitionMst.requisitionDate}"
										pattern="dd-MM-yyyy" /></td>
								<td><c:if test="${cnWsCsRequisitionMst.requisitionTo=='cs'}">
									Central Store
								</c:if> <c:if test="${cnWsCsRequisitionMst.requisitionTo=='ss'}">
									Sub Store
								</c:if></td>
...
<script type="text/javascript">
	$(document)
			.ready(
					function() {
						$('#cnPdRequisitionListTable').DataTable({
							"order" : [ [ 2, "desc" ] ]
						});
						document.getElementById('cnPdRequisitionListTable_length').style.display = 'none';
						//document.getElementById('cnPdRequisitionListTable_filter').style.display = 'none';
