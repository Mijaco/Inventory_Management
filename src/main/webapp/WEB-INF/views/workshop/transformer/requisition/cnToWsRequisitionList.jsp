...
<div class="container-fluid icon-box" style="background-color: #858585;">
	...
	<div class="row" style="background-color: white; padding: 10px; padding-left: 20px; margin: 10px;">
		...
		<div class="col-sm-12 table-responsive">
			<c:if test="${empty cnWsRequisitionMstList}">
			...
			<c:if test="${!empty cnWsRequisitionMstList}">
				...
				<table id="cnWsRequisitionMstListTable"
					...
					<tbody>
						<c:forEach items="${cnWsRequisitionMstList}"
							var="cnWsRequisitionMst">
							<tr>
								<td><a href="${pageContext.request.contextPath}/cnws/showCnWsCsRequisition.do?id=${cnWsRequisitionMst.id}"
									style="text-decoration: none;"><c:out
											value="${cnWsRequisitionMst.requisitionNo}" /></a></td>
								<td><c:out value="${cnWsRequisitionMst.identerDesignation}" /></td>
								<td><fmt:formatDate
										value="${cnWsRequisitionMst.requisitionDate}"
										pattern="dd-MM-yyyy" /></td>
								<td><c:if test="${cnWsRequisitionMst.requisitionTo=='cs'}">
									Central Store
								</c:if> <c:if test="${cnWsRequisitionMst.requisitionTo=='ss'}">
									Sub Store
								</c:if></td>
								...
								<td>
									<div class="action-buttons">
										<a class="btn btn-primary btn-xs" style="border-radius: 6px;"
											href="${pageContext.request.contextPath}/cnws/showCnWsCsRequisition.do?id=${cnWsRequisitionMst.id}">
...
<script type="text/javascript">
	$(document)
			.ready(
					function() {
						$('#cnWsRequisitionMstListTable').DataTable({
							"order" : [ [ 2, "desc" ] ]
						});
						document
								.getElementById('cnWsRequisitionMstListTable_length').style.display = 'none';
