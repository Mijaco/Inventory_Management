	...
	<div class="row"
		...
		<div class="col-md-8">
			...
			<h2 class="center blue col-md-8"
				...
				Your Store Ticket ${ticketNo} is ready.</h2>
		</div>
		<div class="col-xs-4">
			<c:if test="${!empty multipleGPNoList}">
				...
				<c:forEach items="${multipleGPNoList}" var="gatePassNo">
					<a
						href="${pageContext.request.contextPath}/cs/gatePassGenerate.do?gatePassNo=${gatePassNo}"
						target="_blank"> <c:out value="${gatePassNo}"></c:out>
					</a>
				</c:forEach>
			</c:if>
		</div>
			...
			.ready(
					function() {
						...
						document.getElementById('requisitionReportFrame').src = birtAddr
								+ '?__report=desco/procurement/ls_pd_cs_req_st.rptdesign&__toolbar=false&__showtitle=false&__title=&P_TICKET_NO='
								+ param1;
