...
<div class="container-fluid icon-box" style="background-color: #858585;">
	...
	<div class="row" style="background-color: white; padding-left: 10px; margin: 10px; padding-bottom: 10px; padding-top: 10px;">
		...
		<div class="col-sm-12 table-responsive">
			<c:if test="${empty csReturnSlipMstList}">
...
<!-- 					<form method="POST" -->
<%-- 						action="${pageContext.request.contextPath}/cs/received/finalListSearch.do"> --%>
...
<!-- 					</form> -->
				...
				<table id="itemReturnListTable"
					...
					<tbody>
						<c:forEach items="${csReturnSlipMstList}" var="csReturnSlipMst"
							varStatus="loop">
							<tr>
								<td><a href="#" class="blue"
									onclick="postSubmit('${pageContext.request.contextPath}/cs/received/detailsInfo.do',{id:'${csReturnSlipMst.id}'},'POST')">
										<c:out value="${csReturnSlipMst.returnSlipNo}" />
								</a></td>
								<td><fmt:formatDate
										value="${csReturnSlipMst.returnSlipDate}"
										pattern="dd-MM-yyyy" /></td>
								<td><c:out value="${csReturnSlipMst.receiveFrom}" /></td>
								<td id="approved${loop.index}"><c:choose>
										<c:when test="${csReturnSlipMst.approved}">
       										Yes
    									</c:when>
