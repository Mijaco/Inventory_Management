...
<div class="container-fluid icon-box" style="background-color: #858585;">
	...
	<div class="row" style="background-color: white; padding-left: 10px; margin: 10px; padding-bottom: 10px; padding-top: 10px;">
		...
		<div class="col-sm-12 table-responsive">
			<c:if test="${empty csIssuedMstList}">
			...
			<c:if test="${!empty csIssuedMstList}">
				<div class="col-sm-12 pull-right">
					<form method="POST"
						action="${pageContext.request.contextPath}/cs/issued/finalListSearch.do">
				...
				<table id="csIssuedMstListTable"
					...
					<tbody>
						<c:forEach items="${csIssuedMstList}" var="csIssuedMst" varStatus="loop">
							<tr>
								<td><a href="#" class="blue"
									onclick="postSubmit('${pageContext.request.contextPath}/cs/issued/detailsInfo.do',{id:'${csIssuedMst.id}'},'POST')">
										<c:out value="${csIssuedMst.requisitionNo}" />								</a></td>
								<td><fmt:formatDate value="${csIssuedMst.requisitionDate}"
										pattern="dd-MM-yyyy" /></td>
								<td><c:out value="${csIssuedMst.storeTicketNO}" /></td>
								<td><c:out value="${csIssuedMst.gatePassNo}" /></td>
...
<script type="text/javascript">
	...
	$(document)
			.ready(
					function() {
						$('#csIssuedMstListTable').DataTable();
						document.getElementById('csIssuedMstListTable_length').style.display = 'none';
						document.getElementById('csIssuedMstListTable_filter').style.display = 'none';
					});
</script>
