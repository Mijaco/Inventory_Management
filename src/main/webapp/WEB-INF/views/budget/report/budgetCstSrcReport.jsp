...
<div class="row">
		...
		<div class="col-md-2">
			<select class="form-control" name="id" id="srcFund"
				required="required">
				<option value="">Select Budget Funding Source</option>
				<c:forEach items="${srcsOfFund}" var="srcFund">
					<option value="${srcFund.id}">${srcFund.title}</option>
				</c:forEach>
			</select>
		</div>
...
<script type="text/javascript">
	...
	function query() {
		...
		var param2 = $('#srcFund').val();
		if ($.trim(param1) == '' || param1 == null) {
			return;
		} else {
			document.getElementById('requisitionReportFrame').src = birtAddr
					+ '?__report=desco/procurement/budget_cost_source_wise.rptdesign&__toolbar=false&__showtitle=false&__title=&P_DESCO_SESSION='
					+ param1+'&P_COST_SOURCE_ID='+ param2;
		}
