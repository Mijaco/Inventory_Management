...
<div class="container-fluid icon-box" style="background-color: #858585;">
	...
	value="${returnSlipMst.modifiedDate}" pattern="dd-MM-yyyy" /></td>
		...
		<input type="hidden" id="returnTo" value="${returnSlipMst.returnTo}">
		...
		<input type="hidden" id="returnSlipNo" value="${returnSlipMst.returnSlipNo}">
		
		<!-- New Approval Hierarchy :: Start -->
		<!-- Added by: Ihteshamul Alam -->
		<div class="row">
			<div class="col-sm-6 text-left">
				<c:if test="${!empty backManRcvProcs1}">
					<div class="dropup pull-right">
						...
						<ul class="dropdown-menu">
							<c:forEach items="${backManRcvProcs1}" var="backedMan">
									...
									href="javascript:void(0)"> For ${backedMan.buttonName} </a> <c:if
										...
										<ul class="dropdown-menu"
											...
											<c:forEach items="${backedMan.authUser}" var="users">
												<li><a tabindex="-1"
													href="Javascript:backToLower1(${backedMan.stateCode}, '${users.userid}')">
			...
			<div class="col-sm-6 text-center">
				<c:if test="${!empty nextManRcvProcs1}">
					<div class="dropup pull-left">
						...
						<ul class="dropdown-menu">
							<c:forEach items="${nextManRcvProcs1}" var="nextMan">
								<li class="dropdown-submenu"><a tabindex="-1"
									href="javascript:void(0)"> For ${nextMan.buttonName} </a> <c:if
										...
										<ul class="dropdown-menu"
											...
											<c:forEach items="${nextMan.authUser}" var="users">
												<li><a tabindex="-1"
													href="Javascript:forwardToUpper1(${nextMan.stateCode}, '${users.userid}')">
														${users.name}-(${users.designation})</a></li>
											</c:forEach>
										</ul>
									</c:if></li>
				...
				<c:if test="${empty nextManRcvProcs1}">
					<div class="pull-left" style="margin-left: 25px;">
						<button type="button" class="btn btn-primary" id="approveButton"
							onclick="approveing()"
								style="text-decoration: none; border-radius: 6px;">${buttonValue}</button>
					</div>
				</c:if>
			</div>
		</div>
		<!-- New Approval Hierarchy :: End -->
	...
	function forwardToUpper(stateCode){
		if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
			...
			window.location = "${pageContext.request.contextPath}/cn/returnSlip/sendToCn.do?returnSlipNo="+returnSlipNo+"&justification="+justification+"&stateCode="+stateCode;
		}
	}
	...
	function backToLower(stateCode){
		if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
			...
			//window.location = "${pageContext.request.contextPath}/cs/itemRecieved/itemReceivedForwordTo.do?receivedReportNo="+param1+"$justification="+justification;
			window.location = "${pageContext.request.contextPath}/cn/returnSlip/backToCn.do?returnSlipNo="+returnSlipNo+"&justification="+justification+"&stateCode="+stateCode;
		}
	}
	...
	function approveing() {
		if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
			...
			window.location = "${pageContext.request.contextPath}/cn/pd/itemReturnSlipApproved.do?returnSlipNo="+returnSlipNo+"&justification="+justification+"&return_state="+returnStateCode+"&returnTo="+returnTo;
		}
	}
	...
	function forwardToUpper1(stateCode, userid) {
		if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
			...
			var path=contextPath+"/cn/sendToCnCsPdReturn.do";	
	...
	function backToLower1(stateCode, userid) {
		if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
			...
			var path=contextPath+"/cn/backToCnCsPdReturn.do";	
			postSubmit(path, cData, 'POST');
		}
	}
</script>
