		...
		<div class="col-sm-12 table-responsive">
			...
			<c:if test="${!empty localPurchaseMst}">
				<table class="table table-striped table-hover table-bordered">
					...
					<tr>
						<td class="success">Purchase Order No.</td>
						<td>${localPurchaseMst.purchaseOrderNo}</td>
						<td class="success">Supply Date</td>
						<td>
							<fmt:formatDate value="${localPurchaseMst.supplyDate}" pattern="dd-MM-yyyy" />
						</td>
						<td class="success"></td>
						<td></td>
					</tr>
						...
						<c:forEach items="${localPurchaseDtlList}" var="lpDtl"
							...
							<tr>
								...
								<td>
									...
									<button type="button" id="update${loop.index}" onclick="updateLP(${loop.index}, ${lpDtl.id})" style="border-radius: 6px;"
...
<script>
	...
	function updateLP(index, id) {
		...
		$.ajax({
			url : contextPath + '/settings/updateLocalPurchase.do',
