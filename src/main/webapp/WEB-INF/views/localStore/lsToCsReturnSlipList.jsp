				...
				<table id="returnSlipListTable"
					...
					<tbody>
						<c:forEach items="${returnSlipMstList}" var="returnSlipMst" varStatus="loop">
							<tr>
								<td>
								<form id="myForm${loop.index}" action="${pageContext.request.contextPath}/ls/returnSlip/pageShow.do" method="POST">
								...
								<a href="javascript:void(0)" onclick="document.getElementById('myForm${loop.index}').submit();"> <c:out value="${returnSlipMst.returnSlipNo}" /></a>
									</form>		
											</td>

								<td><fmt:formatDate value="${returnSlipMst.returnSlipDate}"
										pattern="dd-MM-yyyy hh:mm:ss a" /></td>
								<td> ${returnSlipMst.receiveFrom}, ${deptName}</td>
								...
								<td>
									<a class="btn btn-xs btn-primary" style="border-radius: 6px;" href="javascript:void(0)" onclick="document.getElementById('myForm${loop.index}').submit();"><i class='fa fa-fw fa-eye'></i>&nbsp;Show</a>
								</td>
							</tr>
						</c:forEach>
					</tbody>
