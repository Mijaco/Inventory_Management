			...
			<table class="col-sm-12 table">
				<tr class="">
					<td class="success">Requisition No: <input type="hidden"
						value="${subStoreRequisitionMst.requisitionNo}"
					...
					<td class="info">
						<c:if test="${subStoreRequisitionMst.requisitionTo=='cs'}">Central Store </c:if>
						<c:if test="${subStoreRequisitionMst.requisitionTo=='ss'}">Sub Store </c:if>
					</td>
				</tr>				
				<tr class="">
					<td class="success">Indenter </td>
					<td class="info">${subStoreRequisitionMst.identerDesignation},  ${subStoreRequisitionMst.deptName}</td>
					<td class="success">Requisition Date:</td>
					<td class="info"><fmt:formatDate
						value="${subStoreRequisitionMst.createdDate}" pattern="dd-MM-yyyy hh:mm:ss a" /></td>
								...
								<td>
									<div class="action-buttons">
										<a href="javascript:void(0)"
											onclick="deleteItem(${subStoreRequisitionDtl.id}, '${subStoreRequisitionMst.requisitionTo}', '${subStoreRequisitionMst.requisitionNo}' )"
											class="btn btn-danger btn-xs" style="border-radius: 6px;">
		...
		<div class="row">
			...
			<div class="col-md-4 col-sm-4 text-center">
				<%-- <a class="btn btn-primary"
					...
					${buttonValue} </a> --%>
				<button onclick="approveing()" type="button" class="btn btn-primary"
					style="text-decoration: none; border-radius: 6px;" id="approveButton">
					${buttonValue}</button>
			</div>
			...
			<div class="modal-dialog modal-lg">
				...
				<div class="modal-content">
					...
					<div class="row"
						...
						<form method="POST"
							action="${pageContext.request.contextPath}/ss/updateItemToSub.do">
							<div class="oe_title">
		...
		<div id="myDialogeBox" class="hide">
			<div class="col-md-12 col-sm-12">
				...
				<form
					...
					<input type="hidden" id="requisitionTo" name="requisitionTo" value="${subStoreRequisitionMst.requisitionTo}">
					<input type="hidden" id="requisitionNo" name="requisitionNo" value="${subStoreRequisitionMst.requisitionNo}">
															...
															<div class="row">
																...
																<div class="form-group col-sm-1 col-xs-12">
																	<input class="form-control quantityRequired" step="0.001" min="0.1"
																		...
																		<strong class="text-danger errQty hide" id="errQty0">Required Qty. can't 0</strong>
																</div>
...
<script src="${pageContext.request.contextPath}/resources/assets/js/localStore/lsRequisitionShowToSub.js"></script>
<script>
...
//		window.location = "${pageContext.request.contextPath}/cs/itemRecieved/itemReceivedForwordTo.do?receivedReportNo="+param1+"$justification="+justification;
		window.location = "${pageContext.request.contextPath}/ls/requisition/sendToLocalToSub.do?requisitionNo="+requisitionNo+"&justification="+justification+"&stateCode="+stateCode;
	}
}
...
function backToLower(stateCode){
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		var requisitionNo = $('#requisitionNo').val();		
//		window.location = "${pageContext.request.contextPath}/cs/itemRecieved/itemReceivedForwordTo.do?receivedReportNo="+param1+"$justification="+justification;
		window.location = "${pageContext.request.contextPath}/ls/requisition/backToLocalToSub.do?requisitionNo="+requisitionNo+"&justification="+justification+"&stateCode="+stateCode;
	}
}
...
function approveing(){
	if( $('#justification').val() == null || $.trim( $('#justification').val() ) == '' ) {
		...
		window.location = "${pageContext.request.contextPath}/ls/itemRequisitionSubmitApprovedLocalToSub.do?requisitionNo="+requisitionNo+"&justification="+justification+"&return_state="+returnStateCode;
	}
}
</script>
