...
<div class="container-fluid icon-box" style="background-color: #858585;">
	...
	<div class="row" style="background-color: white; padding: 10px; margin: 10px;">
		...
		<div class="col-sm-12 table-responsive">
			<c:if test="${empty lsReturnSlipMstList}">
			...
			<c:if test="${!empty lsReturnSlipMstList}">
				<div class="col-sm-12 pull-right">
					<form method="POST"
						action="${pageContext.request.contextPath}/ls/received/finalListSearch.do">
				...
				<table id="itemReturnListTable"
					...
					<tbody>
						<c:forEach items="${lsReturnSlipMstList}" var="lsReturnSlipMst"
							varStatus="loop">
							<tr>
								<td><a href="javascript:void(0)" class="blue"
									onclick="postSubmit('${pageContext.request.contextPath}/ls/received/detailsInfo.do',{id:'${lsReturnSlipMst.id}'},'POST')">
										<c:out value="${lsReturnSlipMst.returnSlipNo}" />
								</a></td>
								<td><fmt:formatDate
										value="${lsReturnSlipMst.returnSlipDate}"
										pattern="dd-MM-yyyy" /></td>
								<td><c:out value="${lsReturnSlipMst.receiveFrom}" /></td>
								<td id="approved${loop.index}"><c:choose>
										<c:when test="${lsReturnSlipMst.approved}">
       										Yes
    									</c:when>
