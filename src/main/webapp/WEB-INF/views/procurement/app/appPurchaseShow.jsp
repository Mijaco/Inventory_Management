		...
		<form
			...
			<div class="table-responsive">
				...
				<c:if test="${stateCode=='100'}">
					<input type="file" name="reviewDoc" id="reviewDoc" class="hidden" />
								...
								name="stateCode" id="sendFor" required="required">
									...
									<c:if test="${!empty approvalHierarchyList}">
										<c:forEach items="${approvalHierarchyList}"
											...
											<option value="${approvalHierarchy.stateCode}">${fn:substring(approvalHierarchy.stateCode, 0, 1)} ${approvalHierarchy.approvalHeader}</option>
										</c:forEach>
									</c:if>
							</select></td>
