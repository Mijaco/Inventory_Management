	...
	<div class="row"
		...
		<div class="col-md-8 ">
			<h2 class="center blue"
				...
				Comprehensive Description of APP for ${descoSession.sessionName}</h2>
						...
						<c:forEach items="${procurementPackageMstList}" varStatus="loop"
							...
							<tr>
								<td>${procurementPackageMst.packageName}<input
									...
									value="${procurementPackageMst.id}" /> <c:set var="appFyId"
										scope="session"
										value="${procurementPackageMst.descoSession.id}" />
								</td>
								...
								<td><fmt:formatNumber type="number" groupingUsed="false"
										value="${procurementPackageMst.estimatedCost/100000}" />
									Million <%-- ${procurementPackageMst.estimatedCost} --%></td>
								...
								<td><input type="text" class="date-picker_my"
									readonly="readonly" name="prepDocInvTender"
									id="prepDocInvTender${loop.index}"
									value="${procurementPackageMst.prepDocInvTender}" /></td>

								<td><input type="text" class="date-picker_my"
									readonly="readonly" name="evaluationOfTender"
									id="evaluationOfTender${loop.index}"
									value="${procurementPackageMst.evaluationOfTender}" /></td>
								<td><input type="text" class="date-picker_my"
									readonly="readonly" name="awardOfContract"
									id="awardOfContract${loop.index}"
									value="${procurementPackageMst.awardOfContract}" /></td>
								<td><input type="text" class="date-picker_my"
									name="tentativeCompletion" readonly="readonly"
									id="tentativeCompletion${loop.index}"
									value="${procurementPackageMst.tentativeCompletion}" /></td>


								<td>${empty procurementPackageMst.currentSessionBudget?'0':procurementPackageMst.currentSessionBudget}</td>
								<td>${empty procurementPackageMst.nextSessionBudget?'0':procurementPackageMst.nextSessionBudget}</td>
								<td><c:if test="${empty procurementPackageMst.confirm}">
										...
										<button class="btn btn-xs btn-danger"
											...
											onclick="postSubmit('${pageContext.request.contextPath}/mps/procurementPackageConfirm.do',{id:'${procurementPackageMst.id}'},'POST')">Confirm!</button>

									</c:if> <c:if
										test="${!empty procurementPackageMst.confirm && procurementPackageMst.purchaseFlag=='0'}">
										<button type="button" id="purchaseBtn${loop.index}"
											onclick="postSubmit('${pageContext.request.contextPath}/app/purchase/getProcurementForm.do',{id:'${procurementPackageMst.descoSession.id}', annexureNo:'${procurementPackageMst.annexureNo}'},'POST')"
											style="border-radius: 6px;"
											class="width-10  btn btn-sm btn-success">
											<i class="ace-icon fa fa-save"></i> <span class="bigger-30">Purchase</span>
										</button>
									</c:if></td>
...
<script type="text/javascript">
	...
	function confirmAll(sessionId){
		var flag = confirm("Do you want to confirm all Package as Final? \nN.B: If yes, you could not further able to Edit any APP Package!!!");
		if (flag == true) {  			
			var contextPath= $('#contextPath').val();
			var path= contextPath + '/mps/procurementPackageConfirmAll.do';
			var cData = {
					id : sessionId
				};
			...
			$.ajax({			
				...
				success : function(data) {
					...
					if(result=='success'){
						var path=contextPath+"/mps/procurementPackageMstList.do";						
						postSubmit(path ,{id: sessionId, packageType:''},'POST');
					} else {
	...
	function updateAPP(n){
		var id=$('#pk'+n).val();
		var contextPath= $('#contextPath').val();
		var path= contextPath + '/mps/updateAPP.do';
